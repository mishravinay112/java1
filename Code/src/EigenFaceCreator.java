...
public class EigenFaceCreator {
  ...
  public double[] readImage(String f) throws FileNotFoundException,
    IllegalArgumentException, IOException {
      ...
      xxxFile file = null;
	  System.out.println("File Name to be checked: "+f);
	 // if(f.equals(null))
	//		JOptionPane.showMessageDialog(null," select the image first...");
      String temp = f.toLowerCase();
      temp = temp.substring(temp.lastIndexOf('.')+1,temp.length());
	  System.out.println("File Extension is : "+temp);
      if (temp.equals("jpg")|| temp.equalsIgnoreCase("jpeg"))
		  file = new JPGFile("C:\\temp\\"+f);
      else if (temp.equals("ppm") || temp.equals("pnm")) file = new PPMFile(f);
      if (file == null)
	  {
		  //JOptionPane.showMessageDialog(null,f+" is not an image file!");
